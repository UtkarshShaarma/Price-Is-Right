...
class ScannerAgent(Agent):
    ...
    def make_user_prompt(self, scraped) -> str:
        """
        Create a user prompt for meta based on the scraped deals provided
        """
    ...
    def scan(self, memory: List[str]=[]) -> Optional[DealSelection]:
        ...
        if scraped:
            ...
            self.log("Scanner Agent is calling LLM using Structured Output")
            client = Together()
            ...
            deal_selection = DealSelection.model_validate_json(result)
            data = json.loads(result)

            # Filter deals with price <= 0
            filtered_deals = [deal for deal in data['deals'] if deal['price'] > 0]

            # Convert filtered deals to JSON string
            filtered_json = json.dumps(filtered_deals, indent=4)
              
            self.log(f"Scanner Agent received {len(deal_selection.deas)} selected deals with price>0 from OpenAI")
